	...
	public static void main(String[] args) {
		...
		try {
			...
			while (!exist1) { // will loop when while is not = true
				...
				sc.nextLine();
				if (!fileName.contains(".doc")) {// to check if file contain.doc? word if not will ask user to type
													// again
					...
					while (!cont) {// if cont is false it will loop// in this case only when user enter Stop then
									// the condition will be triger to true
						System.out.println("Do you want to enter text into file?");
						String choice = sc.next();
						sc.nextLine();
						if (choice.equalsIgnoreCase("yes")) {// to check if user input is yes, if so then will ask user
																// to enter word
